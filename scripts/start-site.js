...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    else if (arg === '--chaos-json') { args.chaosJson = argv[++i]; }
    else if (arg.startsWith('--chaos-json=')) { args.chaosJson = arg.slice('--chaos-json='.length); }
  }
  return args;
}
...
const PRESETS = {
  ...
  all: {
    enabled: true, random_mode: false, seed: 42,
    scenarios: {
